<script>
  ...
  let gifJs = null; // Define gifJs globally

  onMount(() => {
    if (typeof window !== 'undefined') {
      import('gif.js.optimized').then((module) => {
        gifJs = module.default; // Assign gifJs only when loaded in the browser
      });
    }
  ...
  function renderNextFrame(timestamp) {
    ...
    const frameDelay = speedMultiplier === 1
      ? gifFrames[currentFrame].delay * 0.55
      : (gifFrames[currentFrame].delay * 0.55) / speedMultiplier;
    ...
    if (elapsedTime > frameDelay) {
      ...
      if (currentFrame >= gifFrames.length) {
        ...
        if (
          loopOption === 'infinite' ||
          (loopOption === 'specific' && loopsDone < loopCount) ||
          (loopOption === 'once' && loopsDone < 1)
        ) {
